...
namespace ReignOfFear.Content.Systems.FearSystem
{
    ...
    public class FearGlobalNPC : GlobalNPC
    {
        ...
        {
            ApplyOutgoingMods(npc, player, ref modifiers);
        ...
        private static void ApplyOutgoingMods(NPC npc, Player player, ref NPC.HitModifiers modifiers)
        {
            PhobiaSetEffects.Bonus b = PhobiaSetEffects.ComputeEnemyBonus(npc, player);
            if (b.IsZero) return;

            if (b.hp > 0f)
                modifiers.FinalDamage *= 1f / (1f + b.hp);

            if (b.def > 0f)
                modifiers.Defense += npc.defense * b.def;

            if (b.kb > 0f)
                modifiers.Knockback *= 1f - b.kb;
        }
